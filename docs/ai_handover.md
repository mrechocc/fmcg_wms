...
6. 销售订单的“发货”菜单可查看全部物料移动、打开待核准调拨、创建本次调拨，在存在已核准调拨后才可创建销售出库单。
7. “创建销售出库单”使用 ERPNext 原生 Sales Order -> Delivery Note 映射。前端和后端都强制在途订单从客户在途仓出库，且后端会防止单行签收数量超过已核准且未签收的调拨数量。
...
| `fmcg_wms/services/sales_order.py` | 两种交付模式、仓库解析、草稿调拨、多次调拨余额计算 |
...
| `fmcg_wms/api/sales_order.py` | 前端调用的白名单接口：默认中心仓、补建物料移动、当场交付、在途仓预填 |
| `fmcg_wms/public/js/sales_order.js` | 销售订单默认中心仓、发货菜单、自动调拨后留在订单页 |
...
2. 同一销售订单已有已提交的 `fmcg_transit_stock_entry` 时，禁止再次自动调拨。
...
6. 从订单创建销售出库单后，在保存前确认表头和每个明细行仓库显示客户在途仓；提交数量不得超过累计已核准且未签收数量。
