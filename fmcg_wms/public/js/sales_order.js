frappe.ui.form.on("Sales Order", {
  ...
  on_submit(frm) {
    ...
    frm.reload_doc().then(() => {
      if (frm.doc.fmcg_customer_shipment) {
        frappe.set_route("Form", "Customer Shipment", frm.doc.fmcg_customer_shipment);
      }
    });
  },
});
...
function add_transit_shipment_button(frm) {
  if (!frm.doc.fmcg_customer_shipment) {
    frm.add_custom_button(__("\u8865\u5efa\u5728\u9014\u8c03\u62e8\u5355"), () => {
      frappe.confirm(
        __("\u7cfb\u7edf\u5c06\u4e3a\u8fd9\u5f20\u5df2\u63d0\u4ea4\u7684\u9500\u552e\u8ba2\u5355\u8865\u5efa\u4e2d\u5fc3\u4ed3\u81f3\u5ba2\u6237\u5728\u9014\u4ed3\u7684\u8c03\u62e8\u5355\u3002\u662f\u5426\u7ee7\u7eed\uff1f"),
        () => frappe.call({
          method: "fmcg_wms.api.sales_order.create_transit_transfer",
          args: { sales_order_name: frm.doc.name },
          freeze: true,
          freeze_message: "\u6b63\u5728\u8865\u5efa\u5728\u9014\u8c03\u62e8\u5355...",
          callback(response) {
  ...
  frm.add_custom_button(__("\u67e5\u770b\u5728\u9014\u53d1\u8fd0\u5355"), () => {
    frappe.set_route("Form", "Customer Shipment", frm.doc.fmcg_customer_shipment);
  }, __("\u53d1\u8d27"));
}
